<template>
  <div>
    <mt-header class="base-title" title="提现">
    ...
    <div class="account-funds-using-tips">
      ...
      您有
      <span class="withdraw-num">1</span> 条提现申请正在处理中，
      <router-link class="account-funds-change-bankcard" to="/account/withdraw/list">
        点击查看
      </router-link>
    </div>
    <div class="account-funds">
      ...
      <div class="account-funds-form">
        <form action="" method="post" @submit.prevent="handlerSubmit">
          <p class="account-funds-form-amount">提现金额(元)</p>
          <label for="" class="form-amount">
            <span class="form-amount-yen">&yen;</span>
            <input class="form-amount-num" 
            type="text" name='amount' maxlength='11' autocomplete='off' placeholder='最小充值金额为1.00元'
            v-model="withdrawForm.inputAmount" ref="amount"
          ...
          <button class="form-submit"> 提现 </button>
        </form>
      </div>
...
export default {
  ...
  methods: {
    ...
    parseAmount: function(){
      if(isNaN(this.withdrawForm.inputAmount)){
      ...
      console.log('data.inputAmount,', this.withdrawForm.inputAmount)
      this.withdrawForm.inputAmount = this.withdrawForm.inputAmount
        .replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')
        .replace(/\b(0+)/, '');
    },
    handlerSubmit(e){
      const data = this.withdrawForm;
      const { dispatch } = this.$store;
      const withdrawValidator = new Validator(withdrawRules)
      withdrawValidator.validate(data, (errors => {
        if(errors){
          this.isShowError = true;
          this.disableSubmit = true;
        ...
        const data = {
          amount: this.withdrawForm.inputAmount
        }
        dispatch('postAccountWithdraw', data).then(res => {
          console.log('发送action  提现结果=====', res)
          this.thirdpartRes = res
          this.isShowThirdpartForm = true
        }).catch(err => {
          console.log('发送action  提现结果出错=====', err)
